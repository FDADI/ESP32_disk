 ...
 *                        macro define
 *----------------------------------------------------------------------------*/
#define CHAR_USE_HEAP 1 /*如果为1则使用堆 此方法在多线程中较为安全*/
#if CHAR_USE_HEAP == 1  /*使用堆*/
#include <stdlib.h>
#define char_con_malloc malloc
#define char_con_free free
#else
		...
		int  nSrcLength)
{
  ...
  #if CHAR_USE_HEAP == 1 /*使用堆*/
  size_t malloc_len;
  if( nDestLength > nSrcLength )
  {
    malloc_len = nDestLength*sizeof(unsigned int);
  }
  else
  {
    malloc_len = nSrcLength*sizeof(unsigned int);
  }
  unsigned int *unicode32_text = char_con_malloc(malloc_len);
  if( unicode32_text == NULL )
  {
    return -1;
  }
  #else
		...
		int len_utf8)
{
  ...
  #if CHAR_USE_HEAP == 1 /*使用堆*/
  size_t malloc_len;
  if( len_gb2312 > len_utf8 )
  {
    malloc_len = len_gb2312*sizeof(unsigned int);
  }
  else
  {
    malloc_len = len_utf8*sizeof(unsigned int);
  }
  unsigned short *unicode16_text;
  unicode16_text = char_con_malloc(malloc_len);
  if( unicode16_text == NULL )
  {
    return -1;
  }
